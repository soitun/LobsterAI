#!/usr/bin/env bash
# MiniMax Image Understanding - CLI 入口
...
# ---- Handle @file syntax for non-ASCII prompts ----
args=()
if [ $# -gt 0 ]; then
  args=("$@")
  for i in "${!args[@]}"; do
...
if [ ${#NODE_ENV_PREFIX[@]} -gt 0 ]; then
  env "${NODE_ENV_PREFIX[@]}" "$NODE_BIN" "$SCRIPT_DIR/understand_image.js" "${args[@]}"
else
  "$NODE_BIN" "$SCRIPT_DIR/understand_image.js" "${args[@]}"
fi
